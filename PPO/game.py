...
class SnakeGameAI:
    ...
    def play_step(self, action):
        ...
        reward = 0
        game_over = False
        if self._is_collision() or self.frame_iteration > 0.5 * self.h * self.w * len(self.snake):
            game_over = True
            reward -= 10
        ...
        if self.head == self.food:
            ...
            reward += 1 / (max(1.0, get_manhattan_distance(self.head, self.food)))
            self.snake.pop()
    ...
    def _move(self, action):
        ...
        if np.array_equal(action, [1, 0, 0]):
            direction = clock_wise[i]
        elif np.array_equal(action, [0, 1, 0]):
            direction = clock_wise[(i + 1) % 4]
